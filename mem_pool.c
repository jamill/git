...
void *memory_pool_alloc(struct mem_pool_manager *mem_pool_manager, size_t len)
{
	struct mem_pool *p;
	void *r;
...
void *memory_pool_calloc(struct mem_pool_manager *mem_pool_manager, size_t count, size_t size)
{
	size_t len = count * size;
	void *r = memory_pool_alloc(mem_pool_manager, len);
	memset(r, 0, len);
	return r;
}
